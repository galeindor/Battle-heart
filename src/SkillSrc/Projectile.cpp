...
Projectile::Projectile(const sf::Vector2f pos, const sf::Vector2f dest, const int index, std::shared_ptr<Character> target , AnimationParams params)
	:Object(pos, index, params , ProjRowlengths[index], Resources::instance().getProjTexture(index))
{
	setAsTarget(target);
	setAnimation(0);
