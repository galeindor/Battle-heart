...
void Knight::initSkills(const int index)
{
	...
	// create(
	this->addSkill(Skill(Resources::instance().getSkillText(index, i), // basic
		sf::Vector2f(i * (SKILL_RECT_SIZE + 20) + 30, 30),
		skillCooldowns[index][i], _damage,
		singleTarget, !onPlayer, !isActive,1));
	i++;
	this->addSkill(Skill(Resources::instance().getSkillText(index, i), // shield skill
		sf::Vector2f(i * (SKILL_RECT_SIZE + 20) + 30, 30),
		skillCooldowns[index][i], _defend,
		singleTarget, !onPlayer, isActive,1.5f));
	i++;
	this->addSkill(Skill(Resources::instance().getSkillText(index, i), // intimidate
		sf::Vector2f(i * (SKILL_RECT_SIZE + 20) + 30, 30),
		skillCooldowns[index][i], _damage,
		singleTarget, !onPlayer, isActive,1));
